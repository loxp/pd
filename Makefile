...
FAILPOINT_ENABLE  := $$(find $$PWD/ -type d | grep -vE "(\.git|\.retools)" | xargs failpoint-ctl enable)
FAILPOINT_DISABLE := $$(find $$PWD/ -type d | grep -vE "(\.git|\.retools)" | xargs failpoint-ctl disable)
...
static:
	...
	CGO_ENABLED=0 golangci-lint run --no-config --enable misspell --disable errcheck --deadline 120s $$($(PACKAGE_DIRECTORIES))
